...
C  (Parent boxes  are those  that contain more than LIMPAR particles )
        ...
        DO 2 kbox = 1,kp1
          ...
          n2 = NPB1(kbox,2)
          Npbox = n2-n1+1
          IF ( Npbox.GT.LIMPAR ) THEN  ! *  Parent Box
            kparent1 = kparent1 + 1
            ...
            DO 20 n = n1,n2
              ...
              JN = JN + 1 
                IXY(JN) = 1      
              IF ( (ibox.eq.ich2).AND.(jbox.eq.jch2) ) THEN
                IXY(JN) = 2
              ELSE IF ( (ibox.eq.ich3).AND.(jbox.eq.jch3) ) THEN
                IXY(JN) = 3
              ELSE IF ( (ibox.eq.ich4).AND.(jbox.eq.jch4) ) THEN
                IXY(JN) = 4
              END IF
20         CONTINUE
