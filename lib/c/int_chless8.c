/* ../int_chless8.f -- translated by f2c (version 20160102).
...
static integer c_b17 = 262144;

/* Subroutine */ int int_chless8__(integer *kp7, integer *kchildless8)
{
...
/* List 1 (finer levels) */
	kfp = 0;
/* List 3 */
	kb = index33_1.ichildless8[kh - 1];
/* box b index */
	ib = index11_1.ic8[kb - 1];
	jb = index11_1.jc8[kb - 1];
	nb1 = index11_1.npb8[kb - 1];
	nb2 = index11_1.npb8[kb + 65535];
	ipar = (centre_1.xc8[kb - 1] - geom_1.x0) / times_1.ds7 + 1;
	jpar = (centre_1.yc8[kb - 1] - geom_1.y0) / times_1.ds7 + 1;
	kc = 0;
	i__2 = *kp7;
	for (k = 1; k <= i__2; ++k) {
/* Loop over boxes in parents level. */
	    i__ = index11_1.ic7[k - 1];
	    j = index11_1.jc7[k - 1];
...
L21:
	...
	for (m = 1; m <= 4; ++m) {
	    ...
	    for (k = 1; k <= i__2; ++k) {
		ks = lclg[k - 1];
		km = index22_1.ipar7ch8[ks + (m << 14) - 16385];
		if (km == 0) {
		    goto L23;
		}
		++kexam;
...
/* ____________________ */
	...
	near_far__(&c_b17, &ib, &jb, &r89, index11_1.ic9, index11_1.jc9, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
