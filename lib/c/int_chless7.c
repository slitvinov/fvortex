/* ../int_chless7.f -- translated by f2c (version 20160102).
...
static integer c_b22 = 262144;

/* Subroutine */ int int_chless7__(integer *kp6, integer *kchildless7)
{
...
/* List 1 (finer levels) */
	kfp = 0;
/* List 3 */
	kb = index33_1.ichildless7[kh - 1];
/* box b index */
	ib = index11_1.ic7[kb - 1];
	jb = index11_1.jc7[kb - 1];
	nb1 = index11_1.npb7[kb - 1];
	nb2 = index11_1.npb7[kb + 16383];
	ipar = (centre_1.xc7[kb - 1] - geom_1.x0) / times_1.ds6 + 1;
	jpar = (centre_1.yc7[kb - 1] - geom_1.y0) / times_1.ds6 + 1;
	kc = 0;
	i__2 = *kp6;
	for (k = 1; k <= i__2; ++k) {
/* Loop over boxes in parents level. */
	    i__ = index11_1.ic6[k - 1];
	    j = index11_1.jc6[k - 1];
...
L21:
	...
	for (m = 1; m <= 4; ++m) {
	    ...
	    for (k = 1; k <= i__2; ++k) {
		ks = lclg[k - 1];
		km = index22_1.ipar6ch7[ks + (m << 12) - 4097];
		if (km == 0) {
		    goto L23;
		}
		++kexam;
...
/* L25: */
	...
	if (nns > 3450000) {
	    s_wsle(&io___36);
	    do_lio(&c__9, &c__1, "error in int_chless7", (ftnlen)20);
...
/* ____________________ */
	...
	near_far__(&c__65536, &ib, &jb, &r78, index11_1.ic8, index11_1.jc8, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	...
	near_far__(&c_b22, &ib, &jb, &r79, index11_1.ic9, index11_1.jc9, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
