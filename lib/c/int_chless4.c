/* ../int_chless4.f -- translated by f2c (version 20160102).
...
static integer c_b37 = 262144;

/* Subroutine */ int int_chless4__(integer *kp3, integer *kchildless4)
{
    ...
    extern /* Subroutine */ int int_part1__(real *, real *, real *, real *, 
	    ...
	    integer *, integer *);
    static integer n1, n2, listclose[5000000], ib, jb, kb, kc, id, kh, km;
    static real r41, r42;
    static integer nn, ks, np;
    static real r43, r44, r45, r46, r47, r48, r49;
...
/* List 1 (finer levels) */
	kfp = 0;
/* List 3 */
	kb = index33_1.ichildless4[kh - 1];
/* box b index */
	ib = index11_1.ic4[kb - 1];
	jb = index11_1.jc4[kb - 1];
	nb1 = index11_1.npb4[kb - 1];
	nb2 = index11_1.npb4[kb + 255];
	ipar = (centre_1.xc4[kb - 1] - geom_1.x0) / times_1.ds3 + 1;
	jpar = (centre_1.yc4[kb - 1] - geom_1.y0) / times_1.ds3 + 1;
	kc = 0;
	i__2 = *kp3;
	for (k = 1; k <= i__2; ++k) {
/* Loop over boxes in parents level. */
	    i__ = index11_1.ic3[k - 1];
	    j = index11_1.jc3[k - 1];
...
L21:
	...
	for (m = 1; m <= 4; ++m) {
	    ...
	    for (k = 1; k <= i__2; ++k) {
		ks = lclg[k - 1];
		km = index22_1.ipar3ch4[ks + (m << 6) - 65];
		if (km == 0) {
		    goto L23;
		}
		++kexam;
...
/* L25: */
	...
	if (nns > 3450000) {
	    s_wsle(&io___42);
	    do_lio(&c__9, &c__1, "error in int_chless4", (ftnlen)20);
...
/* ____________________ */
	...
	near_far__(&c__1024, &ib, &jb, &r45, index11_1.ic5, index11_1.jc5, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	...
	near_far__(&c__4096, &ib, &jb, &r46, index11_1.ic6, index11_1.jc6, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	...
	near_far__(&c__16384, &ib, &jb, &r47, index11_1.ic7, index11_1.jc7, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	level = 8;
	near_far__(&c__65536, &ib, &jb, &r48, index11_1.ic8, index11_1.jc8, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	level = 9;
	near_far__(&c_b37, &ib, &jb, &r49, index11_1.ic9, index11_1.jc9, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* All close boxes are now childless */
	...
	if (nn > 3450000) {
	    s_wsle(&io___48);
	    do_lio(&c__9, &c__1, "error in int_chless4p", (ftnlen)21);
	    do_lio(&c__3, &c__1, (char *)&nn, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	if (kfp > 60001) {
	    s_wsle(&io___49);
	    do_lio(&c__9, &c__1, "error in int_chless4b", (ftnlen)21);
