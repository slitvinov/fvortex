/* ../int_chless6.f -- translated by f2c (version 20160102).
...
static integer c_b27 = 262144;

/* Subroutine */ int int_chless6__(integer *kp5, integer *kchildless6)
{
...
/* List 1 (finer levels) */
	kfp = 0;
/* List 3 */
	kb = index33_1.ichildless6[kh - 1];
/* box b index */
	ib = index11_1.ic6[kb - 1];
	jb = index11_1.jc6[kb - 1];
	nb1 = index11_1.npb6[kb - 1];
	nb2 = index11_1.npb6[kb + 4095];
	ipar = (centre_1.xc6[kb - 1] - geom_1.x0) / times_1.ds5 + 1;
	jpar = (centre_1.yc6[kb - 1] - geom_1.y0) / times_1.ds5 + 1;
	kc = 0;
	i__2 = *kp5;
	for (k = 1; k <= i__2; ++k) {
/* Loop over boxes in parents level. */
	    i__ = index11_1.ic5[k - 1];
	    j = index11_1.jc5[k - 1];
...
L21:
	...
	for (m = 1; m <= 4; ++m) {
	    ...
	    for (k = 1; k <= i__2; ++k) {
		ks = lclg[k - 1];
		km = index22_1.ipar5ch6[ks + (m << 10) - 1025];
		if (km == 0) {
		    goto L23;
		}
		++kexam;
...
/* L25: */
	...
	if (nns > 3450000) {
	    s_wsle(&io___37);
	    do_lio(&c__9, &c__1, "error in int_chless6", (ftnlen)20);
...
/* ____________________ */
	...
	near_far__(&c__16384, &ib, &jb, &r67, index11_1.ic7, index11_1.jc7, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	level = 8;
	near_far__(&c__65536, &ib, &jb, &r68, index11_1.ic8, index11_1.jc8, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	level = 9;
	near_far__(&c_b27, &ib, &jb, &r69, index11_1.ic9, index11_1.jc9, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* All close boxes are now childless */
	...
	if (nn > 3450000) {
	    s_wsle(&io___43);
	    do_lio(&c__9, &c__1, "error in int_chless6p", (ftnlen)21);
	    do_lio(&c__3, &c__1, (char *)&nn, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	if (kfp > 60001) {
	    s_wsle(&io___44);
	    do_lio(&c__9, &c__1, "error in int_chless6b", (ftnlen)21);
