/* ../int_rest3.f -- translated by f2c (version 20160102).
...
/* Subroutine */ int int_rest3__(integer *kp3)
{
    ...
    static real r21, r22, r31, r32, r33;
    static integer np;
    static real xb, yb;
...
/* All boxes Childless & Parents */
	ib = index11_1.ic3[kb - 1];
	jb = index11_1.jc3[kb - 1];
	xb = centre_1.xc3[kb - 1];
	yb = centre_1.yc3[kb - 1];
	ipar = (xb - geom_1.x0) / times_1.ds2 + 1;
	jpar = (yb - geom_1.y0) / times_1.ds2 + 1;
	i__2 = times_1.kp1;
...
/* L1: */
	...
	near_far__(&c__4, &ipar, &jpar, &r21, index11_1.ic1, index11_1.jc1, &
	...
	near_far__(&c__4, &ib, &jb, &r31, index11_1.ic1, index11_1.jc1, &
...
/* L21: */
	...
	near_far__(&c__16, &ipar, &jpar, &r22, index11_1.ic2, index11_1.jc2, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__16, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar2ch3, index33_1.imark2);
	near_far__(&c__16, &ib, &jb, &r32, index11_1.ic2, index11_1.jc2, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L22: */
	...
	if (n4 > 3450000) {
	    s_wsle(&io___29);
	    do_lio(&c__9, &c__1, "error in rest3b", (ftnlen)15);
	    do_lio(&c__3, &c__1, (char *)&n4, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box_part__(&c__64, &kb, &xb, &yb, &n4, boxexp_1.br3, boxexp_1.bi3)
		;
L88:
	near_far__(&c__64, &ib, &jb, &r33, index11_1.ic3, index11_1.jc3, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
/* CDIR$SHORTLOOP */
	i__2 = kfar;
	for (kbb = 1; kbb <= i__2; ++kbb) {
	    id = listfar[kbb - 1];
	    tempor_1.xbox[kbb - 1] = centre_1.xc3[id - 1];
	    tempor_1.ybox[kbb - 1] = centre_1.yc3[id - 1];
	    tempor_1.prbox[kbb - 1] = poles_1.pr3[id - 1];
	    tempor_1.pibox[kbb - 1] = poles_1.pi3[id - 1];
	    tempor_1.prbox[kbb + 60000] = poles_1.pr3[id + 63];
	    tempor_1.pibox[kbb + 60000] = poles_1.pi3[id + 63];
	    tempor_1.prbox[kbb + 120001] = poles_1.pr3[id + 127];
	    tempor_1.pibox[kbb + 120001] = poles_1.pi3[id + 127];
	    tempor_1.prbox[kbb + 180002] = poles_1.pr3[id + 191];
	    tempor_1.pibox[kbb + 180002] = poles_1.pi3[id + 191];
	    tempor_1.prbox[kbb + 240003] = poles_1.pr3[id + 255];
	    tempor_1.pibox[kbb + 240003] = poles_1.pi3[id + 255];
	    tempor_1.prbox[kbb + 300004] = poles_1.pr3[id + 319];
	    tempor_1.pibox[kbb + 300004] = poles_1.pi3[id + 319];
	    tempor_1.prbox[kbb + 360005] = poles_1.pr3[id + 383];
	    tempor_1.pibox[kbb + 360005] = poles_1.pi3[id + 383];
	    tempor_1.prbox[kbb + 420006] = poles_1.pr3[id + 447];
	    tempor_1.pibox[kbb + 420006] = poles_1.pi3[id + 447];
/* L25: */
	}
	if (kfar > 60001) {
	    s_wsle(&io___31);
	    do_lio(&c__9, &c__1, "error in rest3", (ftnlen)14);
	    do_lio(&c__3, &c__1, (char *)&kbb, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box__(&c__64, &kb, &xb, &yb, &kfar, boxexp_1.br3, boxexp_1.bi3);
/* L20: */
    }
    return 0;
} /* int_rest3__ */
