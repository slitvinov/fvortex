/* ../int_rest7.f -- translated by f2c (version 20160102).
...
/* Subroutine */ int int_rest7__(integer *kp7)
{
    ...
    static real r61, xb, yb;
    static integer np;
    static real r62, r63, r64, r65, r66, r71, r72, r73, r74, r75, r76, r77;
...
/* All boxes Childless & Parents */
	ib = index11_1.ic7[kb - 1];
	jb = index11_1.jc7[kb - 1];
	xb = centre_1.xc7[kb - 1];
	yb = centre_1.yc7[kb - 1];
	ipar = (xb - geom_1.x0) / times_1.ds6 + 1;
	jpar = (yb - geom_1.y0) / times_1.ds6 + 1;
	i__2 = times_1.kp1;
...
/* L1: */
	...
	near_far__(&c__4, &ipar, &jpar, &r61, index11_1.ic1, index11_1.jc1, &
...
/* NT */
	near_far__(&c__4, &ib, &jb, &r71, index11_1.ic1, index11_1.jc1, &
...
/* L21: */
	...
	near_far__(&c__16, &ipar, &jpar, &r62, index11_1.ic2, index11_1.jc2, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__16, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar2ch3, index33_1.imark2);
/* NT */
	near_far__(&c__16, &ib, &jb, &r72, index11_1.ic2, index11_1.jc2, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L22: */
	...
	near_far__(&c__64, &ipar, &jpar, &r63, index11_1.ic3, index11_1.jc3, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__64, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar3ch4, index33_1.imark3);
/* NT */
	near_far__(&c__64, &ib, &jb, &r73, index11_1.ic3, index11_1.jc3, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L23: */
	...
	near_far__(&c__256, &ipar, &jpar, &r64, index11_1.ic4, index11_1.jc4, 
		&kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__256, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar4ch5, index33_1.imark4);
/* NT */
	near_far__(&c__256, &ib, &jb, &r74, index11_1.ic4, index11_1.jc4, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L24: */
	...
	near_far__(&c__1024, &ipar, &jpar, &r65, index11_1.ic5, index11_1.jc5,
		 &kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__1024, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar5ch6, index33_1.imark5);
/* NT */
	near_far__(&c__1024, &ib, &jb, &r75, index11_1.ic5, index11_1.jc5, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L25: */
	...
	near_far__(&c__4096, &ipar, &jpar, &r66, index11_1.ic6, index11_1.jc6,
		 &kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__4096, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar6ch7, index33_1.imark6);
	near_far__(&c__4096, &ib, &jb, &r76, index11_1.ic6, index11_1.jc6, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L26: */
	...
	if (n4 > 3450000) {
	    s_wsle(&io___37);
	    do_lio(&c__9, &c__1, "error in rest7b", (ftnlen)15);
	    do_lio(&c__3, &c__1, (char *)&n4, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box_part__(&c__16384, &kb, &xb, &yb, &n4, boxexp_1.br7, 
		boxexp_1.bi7);
L88:
	near_far__(&c__16384, &ib, &jb, &r77, index11_1.ic7, index11_1.jc7, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
/* CDIR$SHORTLOOP */
	i__2 = kfar;
	for (kbb = 1; kbb <= i__2; ++kbb) {
	    id = listfar[kbb - 1];
	    tempor_1.xbox[kbb - 1] = centre_1.xc7[id - 1];
	    tempor_1.ybox[kbb - 1] = centre_1.yc7[id - 1];
	    tempor_1.prbox[kbb - 1] = poles_1.pr7[id - 1];
	    tempor_1.pibox[kbb - 1] = poles_1.pi7[id - 1];
	    tempor_1.prbox[kbb + 60000] = poles_1.pr7[id + 16383];
	    tempor_1.pibox[kbb + 60000] = poles_1.pi7[id + 16383];
	    tempor_1.prbox[kbb + 120001] = poles_1.pr7[id + 32767];
	    tempor_1.pibox[kbb + 120001] = poles_1.pi7[id + 32767];
	    tempor_1.prbox[kbb + 180002] = poles_1.pr7[id + 49151];
	    tempor_1.pibox[kbb + 180002] = poles_1.pi7[id + 49151];
	    tempor_1.prbox[kbb + 240003] = poles_1.pr7[id + 65535];
	    tempor_1.pibox[kbb + 240003] = poles_1.pi7[id + 65535];
	    tempor_1.prbox[kbb + 300004] = poles_1.pr7[id + 81919];
	    tempor_1.pibox[kbb + 300004] = poles_1.pi7[id + 81919];
	    tempor_1.prbox[kbb + 360005] = poles_1.pr7[id + 98303];
	    tempor_1.pibox[kbb + 360005] = poles_1.pi7[id + 98303];
	    tempor_1.prbox[kbb + 420006] = poles_1.pr7[id + 114687];
	    tempor_1.pibox[kbb + 420006] = poles_1.pi7[id + 114687];
/* L27: */
	}
	if (kfar > 60001) {
	    s_wsle(&io___39);
	    do_lio(&c__9, &c__1, "error in rest7", (ftnlen)14);
	    do_lio(&c__3, &c__1, (char *)&kbb, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box__(&c__16384, &kb, &xb, &yb, &kfar, boxexp_1.br7, boxexp_1.bi7)
		;
/* L20: */
    }
    return 0;
