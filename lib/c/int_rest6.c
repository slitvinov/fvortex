/* ../int_rest6.f -- translated by f2c (version 20160102).
...
/* Subroutine */ int int_rest6__(integer *kp6)
{
    ...
    static real r51, r52, r53, r54;
    static integer np;
    static real r55, r61, r62, r63, r64, r65, r66, xb, yb;
...
/* All boxes Childless & Parents */
	ib = index11_1.ic6[kb - 1];
	jb = index11_1.jc6[kb - 1];
	xb = centre_1.xc6[kb - 1];
	yb = centre_1.yc6[kb - 1];
	ipar = (xb - geom_1.x0) / times_1.ds5 + 1;
	jpar = (yb - geom_1.y0) / times_1.ds5 + 1;
	i__2 = times_1.kp1;
...
/* L1: */
	...
	near_far__(&c__4, &ipar, &jpar, &r51, index11_1.ic1, index11_1.jc1, &
...
/* NT */
	near_far__(&c__4, &ib, &jb, &r61, index11_1.ic1, index11_1.jc1, &
...
/* L21: */
	...
	near_far__(&c__16, &ipar, &jpar, &r52, index11_1.ic2, index11_1.jc2, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__16, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar2ch3, index33_1.imark2);
/* NT */
	near_far__(&c__16, &ib, &jb, &r62, index11_1.ic2, index11_1.jc2, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L22: */
	...
	near_far__(&c__64, &ipar, &jpar, &r53, index11_1.ic3, index11_1.jc3, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__64, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar3ch4, index33_1.imark3);
/* NT */
	near_far__(&c__64, &ib, &jb, &r63, index11_1.ic3, index11_1.jc3, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L23: */
	...
	near_far__(&c__256, &ipar, &jpar, &r54, index11_1.ic4, index11_1.jc4, 
		&kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__256, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar4ch5, index33_1.imark4);
/* NT */
	near_far__(&c__256, &ib, &jb, &r64, index11_1.ic4, index11_1.jc4, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L24: */
	...
	near_far__(&c__1024, &ipar, &jpar, &r55, index11_1.ic5, index11_1.jc5,
		 &kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__1024, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar5ch6, index33_1.imark5);
	near_far__(&c__1024, &ib, &jb, &r65, index11_1.ic5, index11_1.jc5, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L25: */
	...
	if (n4 > 3450000) {
	    s_wsle(&io___35);
	    do_lio(&c__9, &c__1, "error in rest6b", (ftnlen)15);
	    do_lio(&c__3, &c__1, (char *)&n4, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box_part__(&c__4096, &kb, &xb, &yb, &n4, boxexp_1.br6, 
		boxexp_1.bi6);
L88:
	near_far__(&c__4096, &ib, &jb, &r66, index11_1.ic6, index11_1.jc6, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
/* CDIR$SHORTLOOP */
	i__2 = kfar;
	for (kbb = 1; kbb <= i__2; ++kbb) {
	    id = listfar[kbb - 1];
	    tempor_1.xbox[kbb - 1] = centre_1.xc6[id - 1];
	    tempor_1.ybox[kbb - 1] = centre_1.yc6[id - 1];
	    tempor_1.prbox[kbb - 1] = poles_1.pr6[id - 1];
	    tempor_1.pibox[kbb - 1] = poles_1.pi6[id - 1];
	    tempor_1.prbox[kbb + 60000] = poles_1.pr6[id + 4095];
	    tempor_1.pibox[kbb + 60000] = poles_1.pi6[id + 4095];
	    tempor_1.prbox[kbb + 120001] = poles_1.pr6[id + 8191];
	    tempor_1.pibox[kbb + 120001] = poles_1.pi6[id + 8191];
	    tempor_1.prbox[kbb + 180002] = poles_1.pr6[id + 12287];
	    tempor_1.pibox[kbb + 180002] = poles_1.pi6[id + 12287];
	    tempor_1.prbox[kbb + 240003] = poles_1.pr6[id + 16383];
	    tempor_1.pibox[kbb + 240003] = poles_1.pi6[id + 16383];
	    tempor_1.prbox[kbb + 300004] = poles_1.pr6[id + 20479];
	    tempor_1.pibox[kbb + 300004] = poles_1.pi6[id + 20479];
	    tempor_1.prbox[kbb + 360005] = poles_1.pr6[id + 24575];
	    tempor_1.pibox[kbb + 360005] = poles_1.pi6[id + 24575];
	    tempor_1.prbox[kbb + 420006] = poles_1.pr6[id + 28671];
	    tempor_1.pibox[kbb + 420006] = poles_1.pi6[id + 28671];
/* L26: */
	}
	if (kfar > 60001) {
	    s_wsle(&io___37);
	    do_lio(&c__9, &c__1, "error in rest6", (ftnlen)14);
	    do_lio(&c__3, &c__1, (char *)&kbb, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box__(&c__4096, &kb, &xb, &yb, &kfar, boxexp_1.br6, boxexp_1.bi6);
/* L20: */
    }
    return 0;
} /* int_rest6__ */
