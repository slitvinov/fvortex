/* ../int_rest8.f -- translated by f2c (version 20160102).
...
/* Subroutine */ int int_rest8__(integer *kp8)
{
    ...
    static real xb, yb;
    static integer np;
    static real r71, r72, r73, r74, r75, r76, r77, r81, r82, r83, r84, r85, 
	    r86, r87, r88;
...
/* All boxes Childless & Parents */
	ib = index11_1.ic8[kb - 1];
	jb = index11_1.jc8[kb - 1];
	xb = centre_1.xc8[kb - 1];
	yb = centre_1.yc8[kb - 1];
	ipar = (xb - geom_1.x0) / times_1.ds7 + 1;
	jpar = (yb - geom_1.y0) / times_1.ds7 + 1;
	i__2 = times_1.kp1;
...
/* L1: */
	...
	near_far__(&c__4, &ipar, &jpar, &r71, index11_1.ic1, index11_1.jc1, &
...
/* NT */
	near_far__(&c__4, &ib, &jb, &r81, index11_1.ic1, index11_1.jc1, &
...
/* L21: */
	...
	near_far__(&c__16, &ipar, &jpar, &r72, index11_1.ic2, index11_1.jc2, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__16, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar2ch3, index33_1.imark2);
/* NT */
	near_far__(&c__16, &ib, &jb, &r82, index11_1.ic2, index11_1.jc2, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L22: */
	...
	near_far__(&c__64, &ipar, &jpar, &r73, index11_1.ic3, index11_1.jc3, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__64, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar3ch4, index33_1.imark3);
/* NT */
	near_far__(&c__64, &ib, &jb, &r83, index11_1.ic3, index11_1.jc3, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L23: */
	...
	near_far__(&c__256, &ipar, &jpar, &r74, index11_1.ic4, index11_1.jc4, 
		&kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__256, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar4ch5, index33_1.imark4);
/* NT */
	near_far__(&c__256, &ib, &jb, &r84, index11_1.ic4, index11_1.jc4, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L24: */
	...
	near_far__(&c__1024, &ipar, &jpar, &r75, index11_1.ic5, index11_1.jc5,
		 &kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__1024, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar5ch6, index33_1.imark5);
/* NT */
	near_far__(&c__1024, &ib, &jb, &r85, index11_1.ic5, index11_1.jc5, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L25: */
	...
	near_far__(&c__4096, &ipar, &jpar, &r76, index11_1.ic6, index11_1.jc6,
		 &kexam, listexam, &kfar, listfar, &kclose, listclose);
	check_box__(&c__4096, &kclose, listclose, &kexam, listexam, &kpart, 
		listpart, index22_1.ipar6ch7, index33_1.imark6);
	near_far__(&c__4096, &ib, &jb, &r86, index11_1.ic6, index11_1.jc6, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L26: */
	...
	near_far__(&c__16384, &ipar, &jpar, &r77, index11_1.ic7, 
	...
	near_far__(&c__16384, &ib, &jb, &r87, index11_1.ic7, index11_1.jc7, &
		kpart, listpart, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    id = listfar[k - 1];
...
/* L27: */
	...
	if (n4 > 3450000) {
	    s_wsle(&io___39);
	    do_lio(&c__9, &c__1, "error in rest8b", (ftnlen)15);
	    do_lio(&c__3, &c__1, (char *)&n4, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box_part__(&c__65536, &kb, &xb, &yb, &n4, boxexp_1.br8, 
		boxexp_1.bi8);
L88:
	near_far__(&c__65536, &ib, &jb, &r88, index11_1.ic8, index11_1.jc8, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
/* CDIR$SHORTLOOP */
	i__2 = kfar;
	for (kbb = 1; kbb <= i__2; ++kbb) {
	    id = listfar[kbb - 1];
	    tempor_1.xbox[kbb - 1] = centre_1.xc8[id - 1];
	    tempor_1.ybox[kbb - 1] = centre_1.yc8[id - 1];
	    tempor_1.prbox[kbb - 1] = poles_1.pr8[id - 1];
	    tempor_1.pibox[kbb - 1] = poles_1.pi8[id - 1];
	    tempor_1.prbox[kbb + 60000] = poles_1.pr8[id + 65535];
	    tempor_1.pibox[kbb + 60000] = poles_1.pi8[id + 65535];
	    tempor_1.prbox[kbb + 120001] = poles_1.pr8[id + 131071];
	    tempor_1.pibox[kbb + 120001] = poles_1.pi8[id + 131071];
	    tempor_1.prbox[kbb + 180002] = poles_1.pr8[id + 196607];
	    tempor_1.pibox[kbb + 180002] = poles_1.pi8[id + 196607];
	    tempor_1.prbox[kbb + 240003] = poles_1.pr8[id + 262143];
	    tempor_1.pibox[kbb + 240003] = poles_1.pi8[id + 262143];
	    tempor_1.prbox[kbb + 300004] = poles_1.pr8[id + 327679];
	    tempor_1.pibox[kbb + 300004] = poles_1.pi8[id + 327679];
	    tempor_1.prbox[kbb + 360005] = poles_1.pr8[id + 393215];
	    tempor_1.pibox[kbb + 360005] = poles_1.pi8[id + 393215];
	    tempor_1.prbox[kbb + 420006] = poles_1.pr8[id + 458751];
	    tempor_1.pibox[kbb + 420006] = poles_1.pi8[id + 458751];
	}
	if (kfar > 60001) {
	    s_wsle(&io___41);
	    do_lio(&c__9, &c__1, "error in rest8", (ftnlen)14);
	    do_lio(&c__3, &c__1, (char *)&kbb, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	int_box__(&c__65536, &kb, &xb, &yb, &kfar, boxexp_1.br8, boxexp_1.bi8)
		;
/* L20: */
    }
    return 0;
