/* ../int_chless3.f -- translated by f2c (version 20160102).
...
static integer c_b42 = 262144;

/* Subroutine */ int int_chless3__(integer *kp2, integer *kchildless3)
{
    ...
    static real r34, r35, r36, r37, r38, r39;
...
/* List 1 (finer levels) */
	kfp = 0;
/* List 3 */
	kb = index33_1.ichildless3[kh - 1];
/* box b index */
	ib = index11_1.ic3[kb - 1];
	jb = index11_1.jc3[kb - 1];
	nb1 = index11_1.npb3[kb - 1];
	nb2 = index11_1.npb3[kb + 63];
	ipar = (centre_1.xc3[kb - 1] - geom_1.x0) / times_1.ds2 + 1;
	jpar = (centre_1.yc3[kb - 1] - geom_1.y0) / times_1.ds2 + 1;
	kc = 0;
	i__2 = *kp2;
	for (k = 1; k <= i__2; ++k) {
/* Loop over boxes in parents level. */
	    i__ = index11_1.ic2[k - 1];
	    j = index11_1.jc2[k - 1];
...
L21:
	...
	for (m = 1; m <= 4; ++m) {
	    ...
	    for (k = 1; k <= i__2; ++k) {
		ks = lclg[k - 1];
		km = index22_1.ipar2ch3[ks + (m << 4) - 17];
		if (km == 0) {
		    goto L23;
		}
		++kexam;
...
/* L25: */
	...
	if (nns > 3450000) {
	    s_wsle(&io___42);
	    do_lio(&c__9, &c__1, "error in int_chless3", (ftnlen)20);
...
/* _____________________________________ */
	...
	near_far__(&c__256, &ib, &jb, &r34, index11_1.ic4, index11_1.jc4, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	...
	near_far__(&c__1024, &ib, &jb, &r35, index11_1.ic5, index11_1.jc5, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	...
	near_far__(&c__4096, &ib, &jb, &r36, index11_1.ic6, index11_1.jc6, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	...
	near_far__(&c__16384, &ib, &jb, &r37, index11_1.ic7, index11_1.jc7, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	level = 8;
	near_far__(&c__65536, &ib, &jb, &r38, index11_1.ic8, index11_1.jc8, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* ____________________ */
	level = 9;
	near_far__(&c_b42, &ib, &jb, &r39, index11_1.ic9, index11_1.jc9, &
		kexam, listexam, &kfar, listfar, &kclose, listclose);
	i__2 = kfar;
	for (k = 1; k <= i__2; ++k) {
	    ++kfp;
...
/* All close boxes are now childless */
	...
	if (nn > 3450000) {
	    s_wsle(&io___48);
	    do_lio(&c__9, &c__1, "error in int_chless3p", (ftnlen)21);
	    do_lio(&c__3, &c__1, (char *)&nn, (ftnlen)sizeof(integer));
	    e_wsle();
	}
	if (kfp > 60001) {
	    s_wsle(&io___49);
	    do_lio(&c__9, &c__1, "error in int_chless3b", (ftnlen)21);
